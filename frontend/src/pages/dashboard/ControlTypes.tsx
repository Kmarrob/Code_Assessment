...
const ControlTypesContent: React.FC<{ data: DashboardData; companyId: string }> = ({ data, companyId }) => {
...
const blob = await dashboardService.downloadControlTypesPDF(companyId);
      ...
      link.href = url;
      const companyName = data?.company?.name || 'tipos_controle';
...
export const ControlTypes: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {(data) => <ControlTypesContent data={data} companyId={companyId} />}
    </DashboardPageWrapper>
  );
};