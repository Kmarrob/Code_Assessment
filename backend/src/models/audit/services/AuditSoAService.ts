...
export class AuditSoAService {
  ...
  async findByCompany(
    ...
    const docs = await AuditSoA.find(query)
      .sort({ version: -1 })
      .lean();
  ...
  async findActiveByCompany(companyId: string): Promise<IAuditSoA | null> {
    const doc = await AuditSoA.findOne({
      companyId,
      status: 'approved',
    })
      .sort({ version: -1 })
  ...
  async updateControl(
    ...
    const controlIndex = soa.controls.findIndex(
      c => c.clause === clause
    );
  ...
  async exportToSpreadsheet(id: string): Promise<any> {
    ...
    return soa.controls.map(control => ({
      ...
      'Data da última avaliação':
        control.lastAssessmentDate?.toLocaleDateString() || '',
      'Por que não é aplicável?':
        control.justification || '',
      'Implementado?': control.implemented ? 'Sim' : 'Não',
      'Data de implementação':
        control.implementationDate?.toLocaleDateString() || '',
