...
import { GovernanceDocument } from '../models/GovernanceDocument.js';
...
async function seedGovernance() {
  try {
    ...
    await GovernanceDocument.deleteMany({});
    ...
    const insertedStandards = await Standard.find({ code: { $in: standardsData.map(s => s.code) } });
    ...
    const insertedProcedures = await Procedure.find({ code: { $in: proceduresData.map(p => p.code) } });
    ...
    if (instructions.length > 0) {
      await WorkInstruction.insertMany(instructions);
    ...
    const totalInstructions = await WorkInstruction.countDocuments();
    const totalDocuments = await GovernanceDocument.countDocuments();
