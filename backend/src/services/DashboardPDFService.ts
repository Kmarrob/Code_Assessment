...
export class DashboardPDFService {
  ...
  static async generateDashboardPDF(data: DashboardPDFData): Promise<Buffer> {
    ...
    try {
      ...
      let puppeteer;
      let browserOptions: any; // 🔴 CORREÇÃO: Adicionado ': any' para permitir propriedade opcional
  ...
  private static generateReactHTML(data: DashboardPDFData): string {
    ...
    // 🔴 Dados por categoria
    const categoryData = Object.entries(byCategory || {}).map(([key, value]: [string, any]) => ({
    ...
    const conceptData = Object.entries(byCyberConcept || {}).map(([key, value]: [string, any]) => ({
      name: key,
      total: value.total || 0,
      implemented: value.implemented || 0,
      partial: value.partial || 0,
    ...
    // 🔴 Dados por domínio
    const domainData = Object.entries(byDomain || {}).map(([key, value]: [string, any]) => ({
      name: key,
      total: value.total || 0,
      implemented: value.implemented || 0,
      partial: value.partial || 0,
    ...
    // 🔴 Dados por tipo
    const typeData = Object.entries(byType || {}).map(([key, value]: [string, any]) => ({
      name: key,
      total: value.total || 0,
      implemented: value.implemented || 0,
      partial: value.partial || 0,
    ...
    const capabilityData = Object.entries(byCapability || {}).map(([key, value]: [string, any]) => ({
      name: key,
      total: value.total || 0,
      implemented: value.implemented || 0,
      partial: value.partial || 0,
    ...
    const capabilities = [
      'Governança', 'Gestão de ativos', 'Proteção da informação',
      'Gestão de identidade e acesso', 'Relações com fornecedores',
      'Eventos de SI', 'Ameaças e vulnerabilidades', 'Continuidade',
      'Segurança física', 'Desenvolvimento seguro', 'Gestão de redes',
      'Monitoramento e análise', 'Gestão de pessoas', 'Criptografia',
      'Garantia de SI'
    ];
    ...
    const radarData = capabilities.map(cap => {
      ...
      return {
        subject: cap.length > 20 ? cap.substring(0, 20) + '…' : cap,
        fullLabel: cap,
    ...
    const hasRadarData = radarData.some(d => d.Implementado > 0);
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .recharts-wrapper { max-width: 100%; margin: 0 auto; display: block; }
    .recharts-text { fill: #334155 !important; font-size: 7pt !important; }
    .recharts-legend-item-text { font-size: 7pt !important; }
    ...
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    .chart-title { font-size: 9pt; font-weight: bold; color: #475569; text-align: center; margin-bottom: 2pt; }
    .chart-subtitle { font-size: 7pt; color: #94a3b8; text-align: center; margin-bottom: 4pt; }
...
<body>
    ...
    <div class="grid-5">
      ${conceptData.map(concept => `
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4pt; padding: 6pt; text-align: center;">
          ...
          <div style="font-size: 6pt; color: #94a3b8; margin-top: 1pt;">${concept.total > 0 ? Math.round((concept.implemented / concept.total) * 100) : 0}%</div>
        </div>
      `).join('')}
    </div>
    ...
    <div class="grid-4">
      ${domainData.map(domain => `
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4pt; padding: 6pt; text-align: center;">
          ...
          <div style="font-size: 6pt; color: #94a3b8; margin-top: 1pt;">${domain.total > 0 ? Math.round((domain.implemented / domain.total) * 100) : 0}%</div>
        </div>
      `).join('')}
    </div>
  ...
  <div class="page section-break">
    ...
    <div class="grid-3">
      ${typeData.map(type => `
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4pt; padding: 6pt; text-align: center;">
          <div style="font-size: 10pt; font-weight: bold; color: #1e293b;">${type.total}</div>
          ...
          <div style="font-size: 6pt; color: #94a3b8; margin-top: 1pt;">${type.total > 0 ? Math.round((type.implemented / type.total) * 100) : 0}%</div>
        </div>
      `).join('')}
    </div>
  </div>
  ...
  <script type="text/babel">
    (function() {
      ...
      if (pieRoot && pieData.length > 0) {
        ...
        ReactDOM.createRoot(pieRoot).render(React.createElement(PieApp));
      }
      ...
      if (barRoot && barData.length > 0) {
        const BarApp = () => {
          ...
          return React.createElement(
            ...
            React.createElement(
              BarChart,
              { data: data, margin: { top: 10, right: 10, left: 0, bottom: 10 } },
              ...
              React.createElement(YAxis, { tick: { fill: '#94a3b8', fontSize: 9 }, allowDecimals: false }),
              React.createElement(Tooltip, {
                contentStyle: { background: '#1e293b', border: 'none', borderRadius: '4px', color: '#fff', fontSize: '10px' },
                formatter: (v) => [v, 'Controles']
              }),
              React.createElement(Legend, { wrapperStyle: { fontSize: '9px' }, iconSize: 8 }),
              React.createElement(
                Bar,
                { dataKey: 'value', name: 'Controles', radius: [4, 4, 0, 0] },
                data.map((entry, index) => 
                  React.createElement(Cell, { key: 'cell-' + index, fill: entry.color })
                )
              )
        ...
        ReactDOM.createRoot(barRoot).render(React.createElement(BarApp));
      }
      ...
      if (conceptBarRoot && conceptData.length > 0) {
        ...
        const ConceptBarApp = () => {
          return React.createElement(
            ...
            React.createElement(
              BarChart,
              { data: conceptBarData, margin: { top: 10, right: 10, left: 0, bottom: 10 } },
              React.createElement(CartesianGrid, { strokeDasharray: '3 3', stroke: '#e2e8f0' }),
              React.createElement(XAxis, { dataKey: 'name', tick: { fill: '#475569', fontSize: 9 } }),
              React.createElement(YAxis, { tick: { fill: '#94a3b8', fontSize: 9 }, allowDecimals: false }),
              React.createElement(Tooltip, {
        ...
        ReactDOM.createRoot(conceptBarRoot).render(React.createElement(ConceptBarApp));
      }
      ...
      if (domainBarRoot && domainData.length > 0) {
        ...
        const DomainBarApp = () => {
          return React.createElement(
            ...
            React.createElement(
              BarChart,
              { data: domainBarData, margin: { top: 10, right: 10, left: 0, bottom: 10 } },
              React.createElement(CartesianGrid, { strokeDasharray: '3 3', stroke: '#e2e8f0' }),
              React.createElement(XAxis, { dataKey: 'name', tick: { fill: '#475569', fontSize: 9 } }),
              React.createElement(YAxis, { tick: { fill: '#94a3b8', fontSize: 9 }, allowDecimals: false }),
              React.createElement(Tooltip, {
        ...
        ReactDOM.createRoot(domainBarRoot).render(React.createElement(DomainBarApp));
      }

      // Renderizar Radar Chart
      const radarRoot = document.getElementById('radar-chart-root');
      if (radarRoot && radarData.length > 0 && radarData.some(d => d.Implementado > 0)) {
        ...
        const RadarApp = () => {
          return React.createElement(
            ...
            React.createElement(
              RadarChart,
              { data: radarChartData, margin: { top: 10, right: 20, bottom: 10, left: 20 } },
              React.createElement(PolarGrid, { stroke: '#94a3b8' }),
        ...
        ReactDOM.createRoot(radarRoot).render(React.createElement(RadarApp));
      }
