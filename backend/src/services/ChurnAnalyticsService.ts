...
export class ChurnAnalyticsService {
   ...
   */
  async getChurnedClientsList(
    startDate: Date,
    endDate: Date
  ): Promise<ChurnedClient[]> {
    try {
   ...
   */
  async getChurnPrediction(): Promise<{
   ...
   */
  private async getTotalChurned(startDate: Date, endDate: Date): Promise<number> {
   ...
   */
  private async getAverageLifetimeMonths(): Promise<number> {
    ...
    if (result.length === 0) return 6; // Fallback: 6 meses
 ...
 */
export const churnAnalyticsService = new ChurnAnalyticsService();