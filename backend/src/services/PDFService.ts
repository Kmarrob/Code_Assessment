  ...
  static async generateFromHtml(
    ...
    try {
      ...
      if (isProduction) {
        ...
        logger.info('🔄 [generateFromHtml] Usando Chromium do @sparticuz v149.0.0 em produção');
      } else {
        const puppeteerModule = await import('puppeteer') as PuppeteerType;
        puppeteer = puppeteerModule.default || puppeteerModule;
        ...
        logger.info('🔄 [generateFromHtml] Usando Puppeteer local em desenvolvimento');
      }
      ...
      const endTime = Date.now();
      logger.info(`✅ [generateFromHtml] PDF gerado com sucesso em ${endTime - startTime}ms (${pdf.length} bytes)`);

      return Buffer.from(pdf);

    } catch (error) {
      logger.error('❌ [generateFromHtml] Erro ao gerar PDF:', error);
      throw error;
    } finally {
      if (browser) {
        await browser.close();
        logger.debug('🔒 [generateFromHtml] Browser fechado');
      }
    }
  }
}