...
const pdfData = {
   ...
   * Acesso: REP ou ADMIN
   */
  static async generateCategorizationPDF(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      const pdfBuffer = await DashboardPDFService.generateCategorizationPDF(pdfData);

      if (!pdfBuffer || pdfBuffer.length === 0) {
        throw new AppError('Falha ao gerar o PDF de categorização', 500);
      }
      ...
      logger.info(`✅ Categorização PDF gerado para ${companyName} por ${user.email}`);
    } catch (error) {
      logger.error('❌ Erro ao gerar Categorização PDF:', error);
      next(error);
    }
  }
   ...
   * Acesso: REP ou ADMIN
   */
  static async generateControlTypesPDF(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      // 🔴 CHAMAR O MÉTODO ESPECÍFICO DO DashboardPDFService
      const pdfBuffer = await DashboardPDFService.generateControlTypesPDF(pdfData);

      if (!pdfBuffer || pdfBuffer.length === 0) {
        throw new AppError('Falha ao gerar o PDF de tipos de controle', 500);
      }
      ...
      logger.info(`✅ Tipos de Controle PDF gerado para ${companyName} por ${user.email}`);
    } catch (error) {
      logger.error('❌ Erro ao gerar Tipos de Controle PDF:', error);
      next(error);
    }
  }
   ...
   * Acesso: REP ou ADMIN
   */
  static async generateCyberConceptsPDF(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      // 🔴 CHAMAR O MÉTODO ESPECÍFICO DO DashboardPDFService
      const pdfBuffer = await DashboardPDFService.generateCyberConceptsPDF(pdfData);

      if (!pdfBuffer || pdfBuffer.length === 0) {
        throw new AppError('Falha ao gerar o PDF de conceitos cibernéticos', 500);
      }
      ...
      logger.info(`✅ Conceitos Cibernéticos PDF gerado para ${companyName} por ${user.email}`);
    } catch (error) {
      logger.error('❌ Erro ao gerar Conceitos Cibernéticos PDF:', error);
      next(error);
    }
  }
   ...
   * Acesso: REP ou ADMIN
   */
  static async generateCapabilitiesPDF(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      // 🔴 CHAMAR O MÉTODO ESPECÍFICO DO DashboardPDFService
      const pdfBuffer = await DashboardPDFService.generateCapabilitiesPDF(pdfData);

      if (!pdfBuffer || pdfBuffer.length === 0) {
        throw new AppError('Falha ao gerar o PDF de capacidades operacionais', 500);
      }
      ...
      logger.info(`✅ Capacidades Operacionais PDF gerado para ${companyName} por ${user.email}`);
    } catch (error) {
      logger.error('❌ Erro ao gerar Capacidades Operacionais PDF:', error);
      next(error);
    }
  }
   ...
   * Acesso: REP ou ADMIN
   */
  static async generateDomainsPDF(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      // 🔴 CHAMAR O MÉTODO ESPECÍFICO DO DashboardPDFService
      const pdfBuffer = await DashboardPDFService.generateDomainsPDF(pdfData);

      if (!pdfBuffer || pdfBuffer.length === 0) {
        throw new AppError('Falha ao gerar o PDF de domínios', 500);
      }
      ...
      logger.info(`✅ Domínios PDF gerado para ${companyName} por ${user.email}`);
    } catch (error) {
      logger.error('❌ Erro ao gerar Domínios PDF:', error);
      next(error);
    }
  }
}