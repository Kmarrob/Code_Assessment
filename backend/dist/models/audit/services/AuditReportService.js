...
const AuditReport_1 = require("../models/AuditReport");
 ...
 */
function mapToIAuditReport(doc) {
    if (!doc)
        return null;
    return {
        id: doc._id.toString(),
 ...
 */
function mapToIAuditReportArray(docs) {
    if (!docs)
        return [];
...
class AuditReportService {
    ...
    async create(data, createdBy) {
        ...
        const plan = await AuditPlan_1.AuditPlan.findById(data.auditPlanId);
        if (!plan)
        ...
        const report = new AuditReport_1.AuditReport({
            ...data,
            createdBy,
            status: 'draft',
    ...
    async findAll(filters) {
        ...
        const docs = await AuditReport_1.AuditReport.find(query).sort({ createdAt: -1 }).lean();
    ...
    // ============================================================
    async findById(id) {
        const doc = await AuditReport_1.AuditReport.findById(id).lean();
        if (!doc)
            return null;
    ...
    async findByPlanId(auditPlanId) {
        const docs = await AuditReport_1.AuditReport.find({ auditPlanId }).sort({ createdAt: -1 }).lean();
        return mapToIAuditReportArray(docs);
    }
    // ============================================================
    // ATUALIZAR RELATÓRIO
    ...
    async update(id, data, userId) {
        const report = await AuditReport_1.AuditReport.findById(id);
        if (!report)
    ...
    // ============================================================
    async generateAutoReport(planId) {
        const plan = await AuditPlan_1.AuditPlan.findById(planId);
        if (!plan)
            throw new Error('Plano não encontrado');
        const findings = await AuditFinding_1.AuditFinding.find({ auditPlanId: planId });
        ...
        const existingReport = await AuditReport_1.AuditReport.findOne({ auditPlanId: planId });
        if (existingReport) {
...
//# sourceMappingURL=AuditReportService.js.map