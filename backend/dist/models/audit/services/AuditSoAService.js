...
exports.auditSoAService = exports.AuditSoAService = void 0;
const AuditSoA_1 = require("../models/AuditSoA");
// ✅ CORRIGIDO: usar Question do sistema principal em vez de AuditQuestion
const Question_1 = require("../../Question");
class AuditSoAService {
    /**
     * Criar nova Declaração de Aplicabilidade
     */
    async create(data) {
        const soa = new AuditSoA_1.AuditSoA(data);
        // Inicializar controles a partir das perguntas existentes
        if (!data.controls || data.controls.length === 0) {
            // ✅ CORRIGIDO: usar Question.find() em vez de AuditQuestion.find()
            const questions = await Question_1.Question.find({ active: true }).distinct('controlId');
            const controls = questions.map((controlId) => ({
                clause: controlId,
     ...
     */
    async findById(id) {
        const doc = await AuditSoA_1.AuditSoA.findById(id).lean();
        if (!doc)
            return null;
        return {
            id: doc._id.toString(),
    ...
    async findByCompany(companyId, options) {
        ...
        const docs = await AuditSoA_1.AuditSoA.find(query)
            .sort({ version: -1 })
            .lean();
        return docs.map(doc => ({
            id: doc._id.toString(),
    ...
    async findActiveByCompany(companyId) {
        const doc = await AuditSoA_1.AuditSoA.findOne({
            companyId,
            status: 'approved',
        })
            .sort({ version: -1 })
    ...
    async update(id, data) {
        const soa = await AuditSoA_1.AuditSoA.findById(id);
        if (!soa)
            return null;
     ...
     */
    async updateControl(id, clause, data) {
        const soa = await AuditSoA_1.AuditSoA.findById(id);
        if (!soa)
            return null;
     ...
     */
    async archive(id) {
        const doc = await AuditSoA_1.AuditSoA.findByIdAndUpdate(id, {
            status: 'archived',
            updatedAt: new Date(),
        }, { new: true }).lean();
        if (!doc)
    ...
    async getStatistics(id) {
        ...
        return {
            ...
            implementationRate: soa.statistics?.applicable > 0
                ? (soa.statistics.implemented /
                    soa.statistics.applicable) *
                    100
                : 0,
        };
    }
    ...
    async exportToSpreadsheet(id) {
        ...
        return soa.controls.map(control => ({
            ...
            'Motivador - Negócio': control.motivators.business
                ? 'Sim'
                : 'Não',
            'Motivador - Risco': control.motivators.risk
                ? 'Sim'
                : 'Não',
            'Motivador - Jurídico': control.motivators.legal
                ? 'Sim'
                : 'Não',
            'Motivador - Contrato': control.motivators.contract
                ? 'Sim'
                : 'Não',
...
exports.AuditSoAService = AuditSoAService;
exports.auditSoAService = new AuditSoAService();
//# sourceMappingURL=AuditSoAService.js.map