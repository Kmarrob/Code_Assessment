...
// 🔴 NOVO: Serviço específico para PDF do Dashboard de Maturidade
// Este serviço NÃO impacta o PDFService.ts existente (relatório de recomendações)
...
class DashboardPDFService {
    ...
    static async generateDashboardPDF(data) {
        ...
        try {
            ...
            let browserOptions;
            if (isProduction) {
                // Em produção: usar @sparticuz/chromium
                ...
                logger_js_1.logger.info('🔄 DashboardPDF: Usando Chromium do @sparticuz em produção');
            }
            else {
                ...
                logger_js_1.logger.info('🔄 DashboardPDF: Usando Puppeteer local em desenvolvimento');
            }
            ...
            try {
                ...
                logger_js_1.logger.info('✅ DashboardPDF: Gráficos Recharts encontrados');
                await page.evaluate(() => {
                ...
                logger_js_1.logger.info(`📊 DashboardPDF: ${chartCount} gráficos encontrados`);
            }
            catch (error) {
                logger_js_1.logger.warn('⚠️ DashboardPDF: Nenhum gráfico Recharts encontrado');
            }
            ...
            // Gerar PDF em paisagem (A4 landscape)
        ...
        `,
            ...
            logger_js_1.logger.info(`✅ DashboardPDF gerado em ${endTime - startTime}ms (${pdf.length} bytes)`);
            return Buffer.from(pdf);
        }
        catch (error) {
            logger_js_1.logger.error('❌ DashboardPDF: Erro ao gerar PDF:', error);
            throw error;
        }
        finally {
            if (browser) {
    ...
    static generateDashboardHTML(data) {
        ...
        const STATUS_COLORS = {
            'Implementado': '#10b981',
            'Parcialmente implementado': '#f59e0b',
            'Não implementado': '#ef4444',
        ...
        const completionRate = summary.totalControls > 0
            ...
            : 0;
        // Dados para gráficos (versão simplificada para PDF)
        const pieData = [
        ...
        // Gerar dados por conceito cibernético
        const conceptData = Object.entries(byCyberConcept || {}).map(([key, value]) => ({
            name: key,
            total: value.total || 0,
            implemented: value.implemented || 0,
            partial: value.partial || 0,
        ...
        // Gerar dados por domínio
        const domainData = Object.entries(byDomain || {}).map(([key, value]) => ({
            name: key,
            total: value.total || 0,
            implemented: value.implemented || 0,
            partial: value.partial || 0,
        ...
        // Gerar dados por capacidade
        const capabilityData = Object.entries(byCapability || {}).map(([key, value]) => ({
            name: key,
            total: value.total || 0,
            implemented: value.implemented || 0,
            partial: value.partial || 0,
