...
class ReportService {
     ...
     */
    static async getRoadmap(companyId) {
    ...
    static async getPriorizationMatrix(companyId) {
        ...
        try {
            ...
            for (const control of controls) {
                ...
                if (response) {
                    ...
                    if (response.threats && response.threats.length > 0) {
                        vulnerabilities = response.threats.map((t) => `- ${t}`).join('\n');
                    }
                    else if (maturity === 0) {
            ...
            matrixData.sort((a, b) => (priorityOrder[a.priority] || 99) - (priorityOrder[b.priority] || 99));
