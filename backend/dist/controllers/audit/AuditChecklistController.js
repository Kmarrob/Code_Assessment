...
exports.AuditChecklistController = void 0;
const services_1 = require("../../models/audit/services");
class AuditChecklistController {
    // ============================================================
    // BUSCAR CHECKLIST POR PLANO E CONTROLE
    // ============================================================
    async findByPlanAndControl(req, res) {
        try {
            ...
            const checklist = await services_1.auditChecklistService.findByPlanAndControl(auditPlanId, controlId);
            if (!checklist) {
    ...
    async findByPlanId(req, res) {
        try {
            ...
            const checklists = await services_1.auditChecklistService.findByPlanId(auditPlanId);
    ...
    async updateChecklist(req, res) {
        try {
            ...
            const checklist = await services_1.auditChecklistService.updateChecklist(id, questions, userId);
            if (!checklist) {
                res.status(404).json({ success: false, message: 'Checklist não encontrado' });
                return;
            }
    ...
    async complete(req, res) {
        try {
            ...
            const checklist = await services_1.auditChecklistService.complete(id, userId);
            if (!checklist) {
                res.status(404).json({ success: false, message: 'Checklist não encontrado' });
                return;
            }
    ...
    // ============================================================
    async populateWithUserResponses(req, res) {
        try {
            const { auditPlanId } = req.params;
            const userId = req.user?._id?.toString();
            if (!userId) {
            ...
            const populatedCount = await services_1.auditChecklistService.populateAllChecklists(auditPlanId, userId);
            res.status(200).json({
                success: true,
    ...
    async getStats(req, res) {
        try {
            ...
            const stats = await services_1.auditChecklistService.getStats(auditPlanId);
...
//# sourceMappingURL=AuditChecklistController.js.map