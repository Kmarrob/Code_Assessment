...
class ReportController {
    ...
    static async generatePDF(req, res, next) {
        try {
            ...
            let recomendacoes = [];
            try {
                recomendacoes = await RecommendationService_js_1.RecommendationService.getRecommendationsForReport(companyId);
                logger_js_1.logger.info(`✅ ${recomendacoes.length} recomendações carregadas para o PDF`);
            }
            catch (recError) {
                logger_js_1.logger.warn('⚠️ Erro ao carregar recomendações para o PDF:', recError);
            ...
            const companyName = reportData.companyId?.name || 'NOME_DO_CLIENTE';
