...
class DashboardController {
     ...
     * Acesso: REP ou ADMIN
     */
    static async getCriticalControls(req, res, next) {
        try {
            const userId = req.userId;
            if (!userId) {
                throw new errorHandler_js_1.AppError('Usuário não autenticado', 401);
            ...
            if (!companyId) {
                throw new errorHandler_js_1.AppError('Empresa não encontrada no perfil do usuário', 400);
            }
            // Se for REP, garantir que ele só vê a própria empresa
            if (user.role === 'rep') {
                // Já estamos usando a companyId do usuário
            }
            const result = await DashboardService_js_1.DashboardService.getCriticalControlsForAudit(companyId);
            res.json({
                success: true,
                data: result,
            ...
            logger_js_1.logger.error('❌ Erro ao buscar controles críticos para auditoria:', error);
            next(error);
        }
    }
}
