...
class AuditController {
    ...
    static async listLogs(req, res, next) {
        try {
            const { page = 1, limit = 50, action, category, level, userId, userEmail, companyId, success, startDate, endDate, search, } = req.query;
            const filter = {};
            if (action && action !== 'all' && action !== 'todos')
            ...
            const [logs, total] = await Promise.all([
                AuditLog_js_1.AuditLog.find(filter)
                    .sort({ timestamp: -1 })
                    .skip(skip)
                    .limit(Number(limit))
                    .lean(),
                AuditLog_js_1.AuditLog.countDocuments(filter),
            ]);
    ...
    static async getStats(req, res, next) {
        try {
            ...
            const [totalLogs, byCategory, byAction, byLevel, bySuccess, byDay] = await Promise.all([
                AuditLog_js_1.AuditLog.countDocuments({ timestamp: { $gte: daysAgo } }),
                AuditLog_js_1.AuditLog.aggregate([
    ...
    static async getLogById(req, res, next) {
        try {
            const { id } = req.params;
            if (!id) {
                throw new errorHandler_js_1.AppError('ID do log é obrigatório', 400);
            }
            const log = await AuditLog_js_1.AuditLog.findById(id).lean();
            if (!log) {
                throw new errorHandler_js_1.AppError('Log não encontrado', 404);
            }
            res.json({
                success: true,
                data: { log },
    ...
    static async exportLogs(req, res, next) {
        try {
            const { startDate, endDate, category, action, format = 'json', } = req.query;
            const filter = {};
            if (startDate)
                filter.timestamp = { $gte: new Date(startDate) };
            if (endDate)
            ...
            if (action)
                filter.action = action;
            const logs = await AuditLog_js_1.AuditLog.find(filter)
                .sort({ timestamp: -1 })
                .limit(10000)
                .lean();
            if (format === 'csv') {
                ...
                const csvRows = [
                    headers.join(','),
                    ...logs.map(log => headers.map(h => JSON.stringify(log[h] || '')).join(',')),
                ];
