...
const AuditProgram_js_1 = require("../models/audit/models/AuditProgram.js");
const AuditPlan_js_1 = require("../models/audit/models/AuditPlan.js");
const AuditSoA_js_1 = require("../models/audit/models/AuditSoA.js");
const AuditRisk_js_1 = require("../models/audit/models/AuditRisk.js");
const AuditChecklist_js_1 = require("../models/audit/models/AuditChecklist.js");
const AuditFinding_js_1 = require("../models/audit/models/AuditFinding.js");
const AuditDocumentReview_js_1 = require("../models/audit/models/AuditDocumentReview.js");
// ============================================================
...
async function seedAuditData() {
    try {
        ...
        let companies = await Company_js_1.Company.find({});
        ...
        await AuditDocumentReview_js_1.AuditDocumentReview.deleteMany({});
        await AuditFinding_js_1.AuditFinding.deleteMany({});
        await AuditChecklist_js_1.AuditChecklist.deleteMany({});
        await AuditRisk_js_1.AuditRisk.deleteMany({});
        await AuditSoA_js_1.AuditSoA.deleteMany({});
        await AuditPlan_js_1.AuditPlan.deleteMany({});
        await AuditProgram_js_1.AuditProgram.deleteMany({});
        ...
        for (let companyIndex = 0; companyIndex < targetCompanies.length; companyIndex++) {
            ...
            if (!company)
                continue; // ✅ CORREÇÃO: verificar se company existe
            ...
            const program = await AuditProgram_js_1.AuditProgram.create({
            ...
            const plan = await AuditPlan_js_1.AuditPlan.create({
            ...
            const soa = new AuditSoA_js_1.AuditSoA({
                companyId: company._id,
                version: '1.0',
                status: 'approved',
            ...
            const riskPromises = RISK_TEMPLATES.map(async (template, idx) => {
                ...
                const risk = await AuditRisk_js_1.AuditRisk.create({
                    companyId: company._id,
            ...
            const checklistPromises = controlIds.map(async (controlId) => {
                ...
                const questionList = controlQuestions.length > 0
                    ? controlQuestions.slice(0, 3).map(q => ({
                        ...
                        question: q.question || q.pergunta || `Pergunta para controle ${controlId}`,
                        answer: 'NA',
                        observations: '',
                        evidenceIds: [],
                        responsible: responsibleUser.name,
                ...
                const checklist = new AuditChecklist_js_1.AuditChecklist({
                    auditPlanId: plan._id,
                    controlId: controlId,
            ...
            const findingPromises = selectedChecklists.map(async (checklist, idx) => {
                ...
                if (!template)
                    return null; // ✅ CORREÇÃO: verificar template
                const control = controls.find(c => {
                    const cId = c._id ? c._id.toString() : c.id;
                    return cId === checklist.controlId;
                });
                const finding = await AuditFinding_js_1.AuditFinding.create({
                    auditPlanId: plan._id,
            ...
            const documentItems = clauses.map((c) => ({
                ...
                status: ['OK', 'OK', 'OK', 'OK', 'NC_B', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK', 'OK'][Math.floor(Math.random() * 3)],
                observations: Math.random() > 0.8 ? 'Revisão pendente' : '',
                reviewer: responsibleUser.name,
                reviewDate: new Date(2026, 2, 1),
            }));
            const docReview = new AuditDocumentReview_js_1.AuditDocumentReview({
                companyId: company._id,
                auditPlanId: plan._id,
...
// Executar o seed
seedAuditData();
//# sourceMappingURL=seed-audit-data.js.map